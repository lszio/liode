type Slot = { key?: string, slot?: string, value?: unknown, index?: number, start?: number, end?: number, step?: number }
...
export function slice(array: Array<unknown>, start?: number, end?: number, step: Exclude<number, 0> = 1, strict: boolean = true) {
  if (start === undefined) {
